...
class RecommenderMetrics:
    ...
    def mean_average_precision(
        ...
        for user_id, rec_list in recommendations.items():
            ...
            ap = RecommenderMetrics.average_precision(rec_list, relevant, k)
            total_ap += ap
    ...
    def ndcg_at_k(
        ...
        # Calculate DCG
        dcg = RecommenderMetrics.dcg_at_k(recommended, relevant, k, relevance_scores)

        # Calculate ideal DCG (perfect ranking)
        if relevance_scores:
            ideal_order = sorted(relevant, key=lambda x: relevance_scores.get(x, 1.0), reverse=True)
        else:
            ideal_order = list(relevant)

        idcg = RecommenderMetrics.dcg_at_k(ideal_order, relevant, k, relevance_scores)
    ...
    def evaluate_recommendations(
        ...
        for k in k_values:
            ...
            # Hit Rate@K
            results[f'HitRate@{k}'] = cls.hit_rate_at_k(recommendations, relevant_items, k)
...
if __name__ == "__main__":
    ...
    print(f"\nPrecision@5: {RecommenderMetrics.precision_at_k(recommended, relevant, 5):.4f}")
    print(f"Precision@10: {RecommenderMetrics.precision_at_k(recommended, relevant, 10):.4f}")
    print(f"Recall@5: {RecommenderMetrics.recall_at_k(recommended, relevant, 5):.4f}")
    print(f"Recall@10: {RecommenderMetrics.recall_at_k(recommended, relevant, 10):.4f}")
    print(f"AP: {RecommenderMetrics.average_precision(recommended, relevant):.4f}")
    print(f"NDCG@5: {RecommenderMetrics.ndcg_at_k(recommended, relevant, 5):.4f}")
